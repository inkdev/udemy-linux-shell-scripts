...
if [[ "${UID}" -ne 0 ]]
then
	echo "You dont have the root permissions. Please, run it with sudo"
	exit 1
fi
...
if [[ "${#}" -eq 0 ]]
then
	echo "Please, enter your name in the following format: ${0} USER_NAME [COMMENT]... "
	exit 1
fi 
...
if [[ "${?}" -ne 0 ]]
then
	echo 'Your account cannot create completely'
	exit 1
fi
