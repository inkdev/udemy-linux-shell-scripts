...
if [[ "${UID}" -ne 0 ]]
then
  echo "You don't have the root permission. Please, run it with sudo " >&2
  exit 1
fi
#Parse the options
...
#Create an archive if requested to do so
  ...
  then
    ...
    then
      ...
      if [[ "${?}" -ne 0 ]]
      then
        echo "The archive directory ${ARCHIVE_DIR} could not be creted" >&2
        exit 1
      fi
    fi
    ...
    then 
      ...
      #Check to see if the chage commnd succeeded
